...
export default function DepartmentPage() {
  ...
  const otherDepartments = departments.filter((d) => d.slug !== dept.slug).slice(0, 3)
    ...
    <div>
      ...
      <section className="relative bg-gradient-to-br from-primary-950 via-primary-800 to-primary-600 text-white overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28">
          ...
          <div className="flex items-center gap-5 mb-6 animate-fade-in-up">
            ...
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold">{dept.title}</h1>
          </div>
          <p className="text-lg text-slate-200 max-w-2xl animate-fade-in-up">{dept.shortDesc}</p>
        </div>
      </section>
      ...
      <section className="section">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div>
            <span className="text-primary-600 font-semibold tracking-wide uppercase text-sm">Overview</span>
            <h2 className="section-title mt-2">About Our {dept.title} Service</h2>
            <p className="text-slate-600 leading-relaxed mb-8">{dept.overview}</p>
            <Link to="/contact" className="btn-primary">
              Request This Service <FaArrowRight />
            </Link>
          </div>
          <div className="aspect-[4/3] rounded-3xl overflow-hidden shadow-2xl">
            <img src={dept.heroImage} alt={dept.title} className="h-full w-full object-cover" />
          </div>
        </div>
      </section>
      ...
      <section className="bg-slate-50">
        <div className="section">
          <div className="text-center mb-14">
            <span className="text-primary-600 font-semibold tracking-wide uppercase text-sm">Sub-Departments</span>
            <h2 className="section-title mt-2">What's Included in {dept.title}</h2>
            <p className="section-subtitle mx-auto">
              Our {dept.title} department is divided into specialized sub-services to handle
              every requirement with focused expertise.
            </p>
          </div>
      ...
      <section className="section">
        <div className="grid lg:grid-cols-2 gap-16">
          <div>
            <span className="text-primary-600 font-semibold tracking-wide uppercase text-sm">Benefits</span>
            <h2 className="section-title mt-2">Why Choose Our {dept.title} Service</h2>
      ...
      <section className="bg-slate-50">
        <div className="section">
          <div className="text-center mb-14">
            <span className="text-primary-600 font-semibold tracking-wide uppercase text-sm">Explore More</span>
            <h2 className="section-title mt-2">Other Departments</h2>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {otherDepartments.map((d) => (
              <Link key={d.slug} to={`/departments/${d.slug}`} className="card group p-8 flex flex-col">
                ...
                <p className="text-slate-600 text-sm leading-relaxed mb-6 flex-grow">{d.shortDesc}</p>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center text-white">
          ...
          <p className="text-slate-100 max-w-2xl mx-auto mb-8">
            Reach out to {siteConfig.name} today for a free consultation and custom quote
